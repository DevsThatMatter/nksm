...
export default function ChatUI() {
    const initialChatMessages: ChatType = {
        ...
        message: [{ msg: "Hello", mainUser: false }, { msg: "Hello", mainUser: true }]
    };

    const [chatMessages, setChatMessages] = useState<ChatType>(initialChatMessages);
                ...
                <div className="chat-display overflow-y-auto mt-4 max-h-[90%] my-auto">
                    <div className="flex flex-col space-y-4">
                        {chatMessages.message.map((msg, id) => (
                            <div key={id} className={`flex ${msg.mainUser ? "justify-end" : "justify-start"}`}>
                                <div className="p-2">
                                    {msg.msg ? (
                                        <div className={`max-w-md rounded-bl-md rounded-br-md p-2 overflow-hidden flex ${msg.mainUser ? "bg-green-500 rounded-tl-md text-white self-end items-end" : "bg-gray-300 rounded-tr text-black self-start items-start"}`}>
