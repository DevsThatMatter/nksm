...
export default function ChatUI({
  ...
  useChatScroll({
    ...
    count: data?.pages?.[0].items?.messages.length ?? 0
  });
  ...
  useEffect(() => {
    setLockedStatus({ status: data?.pages?.[0].items?.Locked ?? false, accepted: "pending" })
    createLockedStatus(data?.pages?.[0].items?.Locked ?? false)
  ...
  // useChatObserver({ unreadMessages: messageElements, sellerId, buyerId, productId })
          ...
          {status === "pending" ? (
            ...
            <Fragment>
              ...
              {data?.pages.map((page, idx) => (
                <Fragment key={idx}>
                  {page.items?.messages?.map((msg: MessageTypes, j: number) => (
                    <div
                      id={msg.msgId}
                      key={j}
                      className={msg.options ?
                        clsx(
                          `"user-message-${msg.readStatus} ? "true":"false"`,
