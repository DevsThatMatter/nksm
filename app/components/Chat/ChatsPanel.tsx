...
export default function UserChat({ userId }: { userId: string }) {
  ...
  const [userChatHistory, setUserChatHistory] = useState<boolean | null>(null)

  useEffect(() => {
    async function getPrevProductDiscussions() {
      if (userId) {
        const data = await getAllChats(userId);
        setProductDiscussions(data.data);
        setUserChatHistory(data.status);
        console.log("from backend=>", data);
      }
    }
    getPrevProductDiscussions();
    console.log("after setting ", prodcutDiscussions);
  }, [userId]);
          ...
          <SheetContent side="left">
            <SheetHeader>
              <SheetTitle className="flex justify-start">
                {discussions && discussions.length !== 0 ? <Button
                  className="justify-center -top-10" // Adjust the top position as needed
                  size={"icon"}
                  variant="ghost"
                  onClick={() => {
                ...
                >
                  ...
                  : (discussions && <span>Lets have some chats...</span>)}
              </SheetTitle>
            </SheetHeader>
            {
